...
,      (CASE WHEN nvl(BT60.NET_URIAGE, 0) > 0 AND nvl(BT60.NET_URIAGE_ZEN, 0) > 0 THEN NEBIKI ELSE 0 END) AS NET_NEBIKI
,      (CASE WHEN BT60.NET_URIAGE IS NULL THEN 0 ELSE BT60.NET_URIAGE END)
     - (CASE WHEN nvl(BT60.NET_URIAGE, 0) > 0 AND nvl(BT60.NET_URIAGE_ZEN, 0) > 0 THEN NEBIKI ELSE 0 END)  AS NET_URIAGE_AFTER_NEBIKI
/*END*/
/*END*/
...
left outer join (
		SELECT
		  BT65.EIGYO_DT,
		  SUM(COALESCE(BT65.NEBIKI,0)) AS NEBIKI
		FROM
...
/*END*/
		  ...
		  ,BT65SADY BT65
		WHERE
		      BT65.COMPANY_CD = /*companyCd*/'00'
        AND   BT65.EIGYO_DT < /*sysDate*/'20110228'
        AND   BT65.EIGYO_DT BETWEEN /*kikanFromAddDay*/'20110101' AND /*kikanToAddDay*/'20110131'
        AND   BT65.COMPANY_CD    = BN01.COMPANY_CD
        AND   BT65.EIGYO_DT      = BN01.EIGYO_DT
...
/*IF tenpoShubetu == "2" */
        AND   BN01.KBN1 IN ('1', '2')
/*END*/
        AND   BT65.MISE_CD  = BM01.MISE_CD
		GROUP BY
		  BT65.EIGYO_DT
) AS BT65 ON (BT65.EIGYO_DT = BT4YOSAN.YOSAN_DT) OR (BT65.EIGYO_DT = BT60.EIGYO_DT)
left outer join (
		SELECT
		  BT59.EIGYO_DT,
		  SUM(COALESCE(BT59.URI_ZEN_AFT_NBK_/*$zennenDatashubetu*/'DOGETU',0)) AS URIAGE_ZEN_AFTER_NEBIKI
		FROM
			(SELECT MISE_CD FROM BM01TENM BM01
            	WHERE BM01.COMPANY_CD = /*companyCd*/'00'
            	AND   BM01./*$taishoJoken*/'SIBU'_CD = /*hyojiTaisho*/'031'
...
/*END*/
		  ...
		  ,BT59ANZJ BT59
		WHERE
		      BT59.COMPANY_CD = /*companyCd*/'00'
        AND   BT59.EIGYO_DT < /*sysDate*/'20110228'
        AND   BT59.EIGYO_DT BETWEEN /*kikanFromAddDay*/'20110101' AND /*kikanToAddDay*/'20110131'
...
/*IF tenpoShubetu == "2" */
        AND   BN01.KBN1 IN ('1', '2')
/*END*/
        AND   BT59.MISE_CD  = BM01.MISE_CD
		GROUP BY
		  BT59.EIGYO_DT
) AS BT59 ON (BT59.EIGYO_DT = BT4YOSAN.YOSAN_DT) OR (BT59.EIGYO_DT = BT60.EIGYO_DT)
/*END*/
ORDER BY EIGYO_DT
