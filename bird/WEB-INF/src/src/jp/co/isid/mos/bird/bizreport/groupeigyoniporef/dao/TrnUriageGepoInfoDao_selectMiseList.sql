...
,    (
	...
	,      (CASE WHEN URIAGE64.MISE_CD IS NOT NULL THEN COALESCE(BT65.NEBIKI,0) ELSE 0 END) AS NEBIKI
	,      (CASE WHEN URIAGE64.MISE_CD IS NOT NULL THEN COALESCE(BT59.URIAGE_ZEN_AFTER_NEBIKI,0) ELSE 0 END) AS URIAGE_ZEN_AFTER_NEBIKI
	FROM   (
...
/*END*/
	...
	/*IF dataShu.equals("HOSEI") */
		  INNER JOIN (
			...
			WHERE
                ...
                /*IF taishoKikan.equals("KIBETU") */
                    AND     LEFT(EIGYO_DT, 6) between /*kikanFrom*/'200706' AND /*kikanTo*/'200706'
                --ELSE
                    AND     LEFT(EIGYO_DT, 6) = /*kikanFrom*/'200706'
                /*END*/
	...
	/*END*/
		...
		/*IF taishoKikan.equals("KIBETU") */
			AND     LEFT(BT65SADY.EIGYO_DT, 6) between /*kikanFrom*/'200706' AND /*kikanTo*/'200706'
		--ELSE
			AND     LEFT(BT65SADY.EIGYO_DT, 6) = /*kikanFrom*/'200706'
		/*END*/
		GROUP BY
		  BT65SADY.COMPANY_CD,
		  BT65SADY.MISE_CD
	...
	ON (
			(
				BT65.COMPANY_CD = YOSAN44.COMPANY_CD
			AND BT65.MISE_CD    = YOSAN44.MISE_CD
			)
			OR
			(   BT65.COMPANY_CD = URIAGE64.COMPANY_CD
			AND BT65.MISE_CD    = URIAGE64.MISE_CD
			)
	   )
	left outer join (
		SELECT
...
/*END*/
		...
		WHERE
		  ...
		  AND BT59ANZJ.EIGYO_DT <= (SELECT CNT_DATE FROM BR33CDAY WHERE DAY_KBN = '01')
		/*IF taishoKikan.equals("KIBETU") */
			AND     LEFT(BT59ANZJ.EIGYO_DT, 6) between /*kikanFrom*/'200706' AND /*kikanTo*/'200706'
		--ELSE
			AND     LEFT(BT59ANZJ.EIGYO_DT, 6) = /*kikanFrom*/'200706'
		/*END*/
		GROUP BY
		  BT59ANZJ.COMPANY_CD,
		  BT59ANZJ.MISE_CD
	...
	ON (
			(
				BT59.COMPANY_CD = YOSAN44.COMPANY_CD
			AND BT59.MISE_CD    = YOSAN44.MISE_CD
			)
			OR
			(   BT59.COMPANY_CD = URIAGE64.COMPANY_CD
			AND BT59.MISE_CD    = URIAGE64.MISE_CD
			)
	   )
) DATA
