SELECT
   ...
   ,BT37.DENPYO_NO
   ,BT37.SHO_CD_JITU
   ,COALESCE(rtrim(SUB2.SHO_NAME_KJ), TM20.SHO_NAME_KJ) as SHO_NAME_KJ
   ,BT37.SHO_AMOUNT
   ,BT37.NOHIN_TANKA
   ,BT37.URI_KIN
   ,BT37.STAX_URI
   ...
   ,BM32.SHO_CD_DAI
   ,COALESCE(rtrim(TC26.SE_BNR_NAME), TC26_2.SE_BNR_NAME) as SE_BNR_NAME
   ,COALESCE(rtrim(TC03.NISU_NAME), TC03_2.NISU_NAME) as NISU_NAME
   ,CASE
      WHEN BT37.TAX_CD = '01' THEN ' '
      WHEN BT37.TAX_CD = '02' AND STAX_URI <> '3' THEN COALESCE(TC56.KEIGEN_MARK, ' ')
      WHEN BT37.TAX_CD = '99' AND STAX_URI <> '3' THEN COALESCE(TC56.KEIKA_MARK, ' ')
      ELSE ' '
    END AS MARK
FROM
    BT37URTR BT37
        ...
        LEFT OUTER JOIN TC56TAXF TC56 ON (BT37.TAX_CD = TC56.TAX_CD)
        LEFT OUTER JOIN TM20SHOM TM20 ON (BT37.SHO_CD_JITU = TM20.SHO_CD_JITU)
        LEFT OUTER JOIN TC26SEIK TC26_2 ON (TM20.SEIKYU_BNRUI = TC26_2.SEIKYU_BNRUI)
        LEFT OUTER JOIN TM21SHOK TM21 ON (BT37.SHO_CD_JITU = TM21.SHO_CD_JITU AND BT37.NOHIN_UNIT = TM21.UNIT_KBN)
        LEFT OUTER JOIN TC03NISU TC03_2 ON (TM21.SHO_NISUGATA = TC03_2.SHO_NISUGATA)
   ,TM11ONER TM11
   ,(
     SELECT
        U.COMPANY_CD
