...
public class SetCompanyLogicImpl implements SetCompanyLogic {
    /** ロジックID */
    public static final String LOGIC_ID = JikanBetuRefUtil.LOGIC_ID_SET_COMPANY;
	/** LOGIC【表示対象取得】*/
	private GetHyojiTaishoLogic getHyojiTaishoLogic;
	/*
	 *  (非 Javadoc)
	 * @see jp.co.isid.mos.bird.analysis.jikanbeturef.logic.SetCompanyLogic#execute(jp.co.isid.mos.bird.analysis.jikanbeturef.dto.SessionDto, jp.co.isid.mos.bird.analysis.jikanbeturef.dto.RequestDto)
	 */
	public void execute(SessionDto sessionDto, RequestDto requestDto) {
		//１．事前条件処理
		validate(sessionDto, requestDto);
		//２．「全社(前年対象店)(or 全店)」「支部(前年対象店)」「個店(or 店舗)」対象条件プルダウンリスト情報を作成します。
    	...
    	for (int i=0; i<listTaishoJoken.size(); i++) {
            ...
            if(UserType.HONBU.equals(sessionDto.getUserTypeCd())
                    && (TaishoJoken.CODE_ALL.equals(taishoJoken) || TaishoJoken.CODE_SIBU.equals(taishoJoken))) {
                taishoJokenData.setKeyName(taishoJokenData.getKeyName()+"(前年対象店)");
            }
	        ...
	        listHyojiTaisho = (List)logigMap.get(GetHyojiTaishoLogicImpl.RK_LIST);
       		if (listHyojiTaisho != null && listHyojiTaisho.size() > 0) {
                ...
                if (UserType.HONBU.equals(sessionDto.getUserTypeCd()) && TaishoJoken.CODE_SIBU.equals(taishoJoken)) {
                    // 支部の場合はブロック情報を格納する
                    if(logigMap.get("listBlock") != null) {
        ...
        //６．店舗ユーザーログイン時
        if(UserType.TENPO.equals(sessionDto.getUserTypeCd())) {
            CodHyojiTaisho codHyojiTaisho = (CodHyojiTaisho)listHyojiTaisho.get(0);
            String miseCd = codHyojiTaisho.getHyojitaishoCd();
            String miseNameKjClose = codHyojiTaisho.getHyojitaishoNameAddClose();
            
            requestDto.setTenpoName(miseNameKjClose);
            //検索条件で使用
            requestDto.setHyojiTaisho(miseCd);
        }
	}
    /**
     * 事前条件処理
