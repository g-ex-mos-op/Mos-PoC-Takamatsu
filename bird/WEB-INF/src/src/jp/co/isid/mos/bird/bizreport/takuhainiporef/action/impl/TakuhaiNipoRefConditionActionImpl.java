...
import jp.co.isid.mos.bird.bizreport.takuhainiporef.action.TakuhaiNipoRefConditionAction;
...
/**
 * 宅配売上日報の条件部画面アクションクラス
 *
 * @author xjung
 * @modifier xkinu 2013/01/20 海外売上集信対応
 * @modifier xkinu 2013/01/20 MOSCARD画面追加対応
 */
public class TakuhaiNipoRefConditionActionImpl implements TakuhaiNipoRefConditionAction {

    /** 初期処理アクションID */
    public static final String initialize_ACTION_ID = "BBR003A01";
 
    /** 実行アクションID */
    public static final String execution_ACTION_ID = "BBR003A02";
    
    /** アクション【SV検索ボタン】ID */
    public static final String callSvForm_ACTION_ID = "BBR003A03";
        ...
        /*２．SV選択画面から遷移してきた場合。2008/12/09追加 **************************/
        	...
        	try {
	            ...
	            if(getNewSvSearchDto().getReturnKind() == SvSearchDto.RETURNKIND_SELECT){
	                ...
	                //SV-SELECT-4.店舗別画面VIEW_IDをリターンします。(営業日報店舗別画面へ遷移)
    ...
    public String callSvForm()  {
        ...
        getNewSvSearchDto().setNavigationCase(NipoRefConstants.VIEW_ID_T_CONDITION);
