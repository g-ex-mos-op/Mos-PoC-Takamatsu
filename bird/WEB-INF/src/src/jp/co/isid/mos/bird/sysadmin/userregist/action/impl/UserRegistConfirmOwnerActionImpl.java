 ...
 * @author itamoto
 */
public class UserRegistConfirmOwnerActionImpl implements UserRegistConfirmOwnerAction {

//    private static Logger logger_ = Logger.getLogger(UserRegistConfirmOwnerActionImpl.class);
...
/*************************** 2007/02/02 add start xamaruyama ***************************/
        ...
        List arrCompanyCd = new ArrayList(0);
        List arrOption    = new ArrayList(0);
        
        //選択された企業コードを設定します。
        for(int c=0; c<getUserRegistDto().getUserOnerList().size(); c++ ) {
        	UIUserOner eUserOner = (UIUserOner)getUserRegistDto().getUserOnerList().get(c);
        	if(eUserOner.isChecked()) {
        		arrCompanyCd.add(eUserOner.getCompanyCd());
        	}
        }
        //オプション選択の取得（2:オプション機能 3:データ提供）
        if (getUserRegistDto().getOptionFunctionFlg()) {
            arrOption.add("2");
        }
        if (getUserRegistDto().getOptionDataFlg()) {
            arrOption.add("3");
        }
        
        roleParam.put(GetUserRoleLogicImpl.PK_ARR_COMPANY_CD, arrCompanyCd);
        roleParam.put(GetUserRoleLogicImpl.PK_ARR_OPTION, arrOption);
        roleParam.put(GetUserRoleLogicImpl.PK_KEIYAKU_CHOICE, getUserRegistDto().getKeiyakuChoice());
        
        // １．ロールコードを取得するロジック（）に入り、結果をDTOにセットする（List型）。
...
/*************************** 2007/02/02 add end xamaruyama ***************************/
    ...
    public String cancel() {
        return userRegistDto.editOwner_VIEW_ID;
    }
