...
,      MH.GAS_FLG 
,      MH.WATER_FLG 
,      CASE WHEN SUBSTR(MH.OPEN_DT, 1, 6) >= /*targetYMZengetu*/'200807' THEN 'V' ELSE '' END NEW_OPEN
FROM 
       (SELECT H.HONBU_CD 
       ,      H.HONBU_NAME 
       ,      H.JIGYOU_CD 
       ...
       FROM 
              ...
              AND             BM68.COMPANY_CD = BD08.COMPANY_CD
              AND             BM68.MISE_CD   = BD08.MISE_CD
              AND             BM68.METER_KBN = BD08.METER_KBN    
...
/*END*/
              AND             BD08.METER_YM  = /*targetYMZengetu*/'200807'
              AND             BD08.METER_KBN = /*meterKbn*/'0'
              AND             (  
                             ((BD08.ELECTRIC_AMT >=100000)  AND BM68.ELECTRIC_FLG ='1')  
                          OR ((BD08.POWER_AMT >=100000)     AND BM68.POWER_FLG    ='1')  
                          OR ((BD08.GAS_AMT >=10000 )       AND BM68.GAS_FLG      ='1')  
                          OR ((BD08.WATER_AMT >=1000 )      AND BM68.WATER_FLG    ='1')  
                              )  
              )AS H 
       LEFT JOIN 
              (SELECT MISE_CD 
              ...
              FROM 
                      BD07METM AS M 
              WHERE 
                      M.COMPANY_CD = /*companyCd*/'00'
              AND     M.METER_YM = /*targetYM*/'200807'
              AND     M.METER_KBN = /*meterKbn*/'0'
              ) AS M 
       ON   (H.MISE_CD = M.MISE_CD)
       ) AS MH 
       LEFT JOIN 
       (SELECT M.MISE_CD 
       ,       M.ELECTRIC_METER 
       ...
       WHERE 
               M.COMPANY_CD = /*companyCd*/'00'
       AND     M.METER_YM = /*targetYMZengetu*/'200807'
       AND     M.METER_KBN = /*meterKbn*/'0'
       ) AS M 
       ON (MH.MISE_CD = M.MISE_CD )
ORDER BY MH.JIGYOU_CD, MH.SLAREA_CD, MH.SIBU_CD, MH.MISE_CD  
