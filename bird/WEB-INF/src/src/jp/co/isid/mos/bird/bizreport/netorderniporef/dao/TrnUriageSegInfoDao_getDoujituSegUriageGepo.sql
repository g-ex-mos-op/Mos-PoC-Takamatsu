...
from
    ...
    (select
        ...
        sum(bd53.misecnt_kbn_nsum) as misecnt_kbn_nsum,
        sum(bd53.misecnt_kbn_ntake) as misecnt_kbn_ntake,
        sum(bd53.misecnt_kbn_ntaku) as misecnt_kbn_ntaku,
     ...
     from
        ...
        join
            (select
                 ...
                 max(bd53.open_kbn_zen_dojitu) as open_kbn_zen,
                 max(bd53.misecnt_dojitu_nsum) as misecnt_kbn_nsum,
                 max(bd53.misecnt_dojitu_ntake) as misecnt_kbn_ntake,
                 max(bd53.misecnt_dojitu_ntaku) as misecnt_kbn_ntaku,
                 sum(bd53.uriage_dojitu) as uriage,
                 sum(bd53.kyakusu_dojitu) as kyakusu,
                 sum(bd53.uriage_zen_dojitu) as uriage_zen,
                 sum(bd53.kyakusu_zen_dojitu) as kyakusu_zen,
                 sum(bd53.uriage_dojitu_nsum) as uriage_nsum,
                 sum(bd53.kyakusu_dojitu_nsum) as kyakusu_nsum,
                 sum(bd53.uriage_zen_dojitu_nsum) as uriage_nsum_zen,
                 sum(bd53.kyakusu_zen_dojitu_nsum) as kyakusu_nsum_zen,
                 sum(bd53.uriage_dojitu_ntake) as uriage_ntake,
                 sum(bd53.kyakusu_dojitu_ntake) as kyakusu_ntake,
                 sum(bd53.uriage_zen_dojitu_ntake) as uriage_ntake_zen,
                 sum(bd53.kyakusu_zen_dojitu_ntake) as kyakusu_ntake_zen,
                 sum(bd53.uriage_dojitu_ntaku) as uriage_ntaku,
                 sum(bd53.kyakusu_dojitu_ntaku) as kyakusu_ntaku,
                 sum(bd53.uriage_zen_dojitu_ntaku) as uriage_ntaku_zen,
                 sum(bd53.kyakusu_zen_dojitu_ntaku) as kyakusu_ntaku_zen
             from
                 BD53NGEP as bd53
                 join
             ...
             where
                 bd53.eigyo_dt = /*kikanFrom*/'200605'
             /*IF tenShu.equals("ALL")*/
                 and (bd53.open_kbn = 1 or bd53.open_kbn_zen_dojitu = 1)
             -- ELSE
                 and bd53.open_kbn = 1
             /*END*/
             and bd53.oldm_flg = '0'
