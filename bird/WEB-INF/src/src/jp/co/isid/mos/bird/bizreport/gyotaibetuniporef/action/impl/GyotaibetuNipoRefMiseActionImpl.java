...
import jp.co.isid.mos.bird.bizreport.gyotaibetuniporef.action.GyotaibetuNipoRefMiseAction;
 ...
 */
public class GyotaibetuNipoRefMiseActionImpl extends CsvOutput2ActionImpl implements
GyotaibetuNipoRefMiseAction {
    /** 初期処理アクションID */
    public static final String initialize_ACTION_ID   = ACTION_ID+"1";
    /** 実行アクションID */
    public static final String enforcement_ACTION_ID  = ACTION_ID+"2";
    /** 会社プルダウン変更アクションID */
    public static final String reSearchTaishoSibuList_ACTION_ID  = ACTION_ID+"3";
    /** CSVダウンロードアクションID */
    public static final String downloadCsvMise_ACTION_ID  = ACTION_ID+"4";
    /** 個店情報ポータルへ遷移アクションID */
    public static final String selectMise_ACTION_ID  = ACTION_ID+"5";
    /** 営業日報画面表示アクションID */
    public static final String dispEigyoNipo_ACTION_ID  = ACTION_ID+"6";
    /** 宅配日報画面表示アクションID */
    public static final String dispTakuhai_ACTION_ID  = ACTION_ID+"7";
    /** アクション【SV検索ボタン】処理ID */
    ...
    /** アクション【ネット注文画面遷移】ID */
    public static final String callNetorder_ACTION_ID = GyotaibetuNipoConstants.VIEW_ID+"A30";
    ...
    public String initialize() {
        ...
        //２．SV選択画面から遷移してきた場合。2008/12/09追加 **************************/
            ...
            try {
	        	...
	        	GyotaibetuNipoRefResultDto saveResultDto = (GyotaibetuNipoRefResultDto)getGyotaibetuNipoRefMapResultDto().getResultDto(windowId);
	        	if(saveResultDto != null) {
	        		setGyotaibetuNipoRefResultDto(saveResultDto);
	        	}
        ...
        //５．検索結果が存在する場合、
        if(gyotaibetuNipoRefResultDto.isExistDataFlg()) {
        	String[] linkParams = GyotaibetuNipoCommon.getLinkParams(resultParamDto.getLinkParams());
        	//DTO【結果情報】屋号名称へ日報共通DTO【結果条件】リンクパラメータから名称を取得し設定します。
        	gyotaibetuNipoRefResultDto.setYagoName(linkParams[3]);
        }
    ...
    public String reSearchTaishoSibuList() {
        ...
        if(!getGyotaibetuNipoRefResultDto().isExistDataFlg()) {
    ...
    public String callSvForm()  {
        ...
        getNewSvSearchDto().setNavigationCase(NipoRefConstants.VIEW_ID_Y_MISE);
	...
	public String callMoscard() {
        ...
        resultParamDto.setSubTagKbn(BizReportConstants.SUB_TAG_0);
        //２．MOSCARD画面店舗一覧画面(店舗別)VIEW_IDをリターンします。
	...
	public String callNetorder() {
        ...
        NipoRefUtil.callScreenInitialize(
        		getNipoRefConditionDto(), searchParamDto, resultParamDto, isNewWindowFlg());
        //３．ネット注文画面店舗一覧画面(店舗別)VIEW_IDをリターンします。
	...
	private void searchMiseList(NipoRefConditionParameterDto parameterDto){
        ...
        //２．DTO【結果条件】.集計区分＝SV指定(担当店一覧)以外の場合
        	...
        	String sibuCd = parameterDto.getSibuCd();
            if (CommonUtil.isNull(sibuCd)) {
            	//存在しない場合はNotNullExceptionを発生させます。
                throw new NotNullException(GyotaibetuNipoConstants.MSG_SIBU_CD, "taishoSibu", 0);
            }
        }
        //４．Try開始 検索処理を行います。
		try{
	        //1.LOGIC【検索条件取得】を実行し、戻り値Map[検索条件]を取得します。
			Map paramMap = getGyotaibetuNipoSearchLogic().getSearchData( parameterDto );
			//2.処理1のMap[検索条件]へ下記の値を格納します。
			String[] linkParams = GyotaibetuNipoCommon.getLinkParams(resultParamDto.getLinkParams());
			paramMap.put( GyotaibetuNipoConstants.SIBU_CD, parameterDto.getSibuCd() );
			...
			//3.LOGIC【売上情報取得】を実行し、Map[売上情報取得]を取得します。
			List miseList = getGyotaibetuNipoRefMiseLogic().execute(paramMap);
            //4.SV情報セット
            String svCd   = (String) paramMap.get(GyotaibetuNipoConstants.SV_CD);
            String svName = (String) paramMap.get(GyotaibetuNipoConstants.SV_NAME);
            //5.DTO【検索条件】.SVコードへMap[検索条件]のSVコード(前ゼロ付加済)を再設定します。
            parameterDto.setSvCd(svCd);
            //6.DTO【検索条件】.SV名称へMap[検索条件]のSV名称を設定します。
            parameterDto.setSvName(svName);
			//7.取得データの画面表示用の編集
			gyotaibetuNipoRefResultDto.setListView(
					getGyotaibetuNipoRefMiseLogic().editViewDataList(miseList, parameterDto));
	        //8.DTO【結果情報】.該当データ存在判断フラグへtrueを設定する。
   	    	gyotaibetuNipoRefResultDto.setExistDataFlg(true);
   	    	//9.DTO【結果情報】.検索タイプへTaishoJoken.CODE_MISEを設定する。
   	    	gyotaibetuNipoRefResultDto.setSearchListType(TaishoJoken.CODE_MISE);
	        //10.DTO【結果情報】換算フラグへDTO【検索条件】.換算フラグを設定する。
   	    	gyotaibetuNipoRefResultDto.setKansanFlg(parameterDto.isKansan());
	        //11.日報共通DTO【結果条件】へDTO【検索条件】を設定します。
	        setNipoRefResultParameterDto(parameterDto);
	        //12.DTO【Map結果情報】.日報共通DTO【結果条件】へDTO【検索条件】を設定します。
	        getGyotaibetuNipoRefMapResultDto().setParameterDto(parameterDto);
	    	//13.DTO【Map結果情報】.DTO【結果情報】へDTO【検索条件】.ウィンドウIDをキーに、
	        //DTO【結果情報】を格納します。
	        getGyotaibetuNipoRefMapResultDto().setResultDto(
	        		parameterDto.getWindowId(), gyotaibetuNipoRefResultDto);
	        //14.日報共通DTO【検索条件】.再検索制御フラグへtrueを設定します。
    ...
    private void getListTaishoSibu(NipoRefConditionParameterDto parameterDto, boolean isExThrow) {
    	...
    	//３．処理２でNotExistExceptionが発生した場合、下記の処理を行います。
            ...
            getGyotaibetuNipoRefResultDto().clear();
	...
	/**
	 * LOGIC【検索条件取得】
	 * @param gyotaibetuNipoSearchLogic
	 */
