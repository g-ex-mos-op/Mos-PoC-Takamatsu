 ...
 * @author xnkusama
 */
public class ShubetuSuiiRefGepoCsvOutputLogicImpl implements CsvOutputLogic {
    /** ロジックID */
    public static final String LOGIC_ID = "BDT003L05";
    ...
    private List makeHeader(ShubetuSuiiRefReqDto dto) {
        ...
        if (dto.getAppDate().substring(0, 6).equals(dto.getKikan())) {
            listHeader1.add("");
            listHeader1.add("※合計、平均に当年当月は含まれません。 ");
        }
        
        // ２行目
        List listHeader2 = new ArrayList();
        listHeader2.add("対象期間：");
        listHeader2.add(dateFormatterYM.format(dto.getTaishoKikanCsv(), true) + "から" + dateFormatterYM.format(dto.getKikan(), true));
        listHeader2.add("");
...
//        listRowHeader.add("外販・その他 前年比(客数)");
    ...
    private List makeMainData(ShubetuSuiiRefReqDto dto) {
        ...
        for (Iterator ite = dto.getListGepoData().iterator(); ite.hasNext();) {
            UIShubetuSuii entity = (UIShubetuSuii) ite.next();
            List listRow = new ArrayList();
            listRow.add(dateFormatterYM.format(entity.getEigyoDt(), true));
            //売上
