...
public class UIViewTutatu implements TrnInfoAccessControl, FileAccessInfoEntity {
...
//    public static final String matchingFileName_COLUMN = "MATCHING_FILE_NAME";
...
//    }
    ...
    public boolean isExistAttachFile() {
        return isExistAttachFile1() || isExistAttachFile2() || isExistAttachFile3();
    }
    ...
    public int getListAttachFileSize() {
        ...
        if(getKanrenFileList() != null){
            size = size + getKanrenFileList().size();
        }
    ...
    public List getListAttachFile() {
        ...
        if (!isNull(getAttachName1())) {
            SelectItem item  =new SelectItem("1", splitString(getAttachName1()));
            list.add(item);
        }
        if (!isNull(getAttachName2())) {
            SelectItem item  =new SelectItem("2", splitString(getAttachName2()));
            list.add(item);
        }
        if (!isNull(getAttachName3())) {
            SelectItem item  =new SelectItem("3", splitString(getAttachName3()));
            list.add(item);
        }

        if(getKanrenFileList() != null){
            for (int i=0; i<getKanrenFileList().size(); i++) {
                UIDocSearch uiDoc = (UIDocSearch)getKanrenFileList().get(i);
                SelectItem item  =new SelectItem("4-" + i, splitString(uiDoc.getTitle()));
                list.add(item);
            }
        }
        return list;
     ...
     * 添付ファイル・関連文書ダウンロード用プルダウン
     * （全文検索以外）
     * ファイルタイトルを60バイト区切り表示する
     * @return
     */
    public List getListAttachFileFullTextSearch() {
        List list = new ArrayList();
        ...
        if(getKanrenFileList() != null){
            for (int i=0; i<getKanrenFileList().size(); i++) {
                UIDocSearch uiDoc = (UIDocSearch)getKanrenFileList().get(i);
                SelectItem item  =new SelectItem("4-" + i, uiDoc.getTitle());
                list.add(item);
            }
        }
        return list;
    ...
    private String splitString(String str) {
        ...
        for (int i = 25; i < str.trim().length(); i++) {
            retStr = str.trim().substring(0, i);
            if (retStr.getBytes().length >= 50) {
                break;
            }
        }
        return retStr;
...
//        this.matchingFileName = matchingFileName;
    ...
    /**
     * @return kanrenFileList を戻します。
     */
    public List getKanrenFileList() {
        return kanrenFileList;
    }
    /**
     * @param kanrenFileList kanrenFileList を設定。
     */
    public void setKanrenFileList(List kanrenFileList) {
        this.kanrenFileList = kanrenFileList;
    }
    /* (非 Javadoc)
