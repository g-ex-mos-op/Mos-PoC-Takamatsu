...
import jp.co.isid.mos.bird.entry.common.code.UserType;
...
import jp.co.isid.mos.bird.entry.nationalregist.dao.UIEntryDateDao;
import jp.co.isid.mos.bird.entry.nationalregist.dao.UIEntryMstDao;
import jp.co.isid.mos.bird.entry.nationalregist.dao.UIEntrySelectionDao;
 ...
 * @author xjung
 * 
 * 更新日:2012/06/25 xkinu オプション登録仕様変更(歯抜け)対応
 *    (※修正前は歯抜けになっていた場合は順番を詰めて登録する仕様だったため。)
 */
public class UpdateEntryLogicImpl implements UpdateEntryLogic {
    /** 全国大会マスタ情報登録ロジックID */
    public static final String LOGIC_ID = "BEN014L04";
    ...
    public void execute(NationalRegistDto dto) {
        ...
        if(NationalRegistConstants.EDIT_MODE_INSERT == dto.getEditMode()) {
            executeInsert(dto);
        // 編集(更新)
        } else if(NationalRegistConstants.EDIT_MODE_UPDATE == dto.getEditMode()) {
            executeUpdate(dto);
        // 削除(更新)
     ...
     * @param dto 全国大会マスタ登録情報DTO
     */
    private void executeInsert(NationalRegistDto dto) {
        // エントリー年 = 開催開始日の年度
        String entryYear = createEntryYear(dto.getUiEntryMst().getOpenFrom());
        // エントリー回取得
        String entryKai = createEntryKai(EntryCode.NATIONAL, entryYear);
        // 現在日付取得
        ...
        // エントリーコード、年、回設定
        uiEntryMst.setEntryCd(EntryCode.NATIONAL);
        uiEntryMst.setEntryYear(entryYear);
        uiEntryMst.setEntryKai(entryKai);
        // 削除フラグ設定
        uiEntryMst.setSakujoFlg(NationalRegistConstants.SAKUJO_FLG_OFF);
        ...
        // エントリーコード、年、回
        uiEntryDate.setEntryCd(EntryCode.NATIONAL);
        ...
        // 開催日登録
        insertEntryDate(uiEntryDate, UserType.KANRI,
            NationalRegistConstants.DAY_KBN_OPEN,
            uiEntryMst.getOpenFrom(), uiEntryMst.getOpenTo());

        // オーナー：表示日登録
        insertEntryDate(uiEntryDate, UserType.ONER,
            NationalRegistConstants.DAY_KBN_DISP,
            uiEntryMst.getOnerDispFrom(), uiEntryMst.getOnerDispTo());

        // オーナー：申込日登録
        insertEntryDate(uiEntryDate, UserType.ONER,
            NationalRegistConstants.DAY_KBN_ENTRY,
            uiEntryMst.getOnerEntryFrom(), uiEntryMst.getOnerEntryTo());

        // 本部：表示日登録
        insertEntryDate(uiEntryDate, UserType.HONBU,
            NationalRegistConstants.DAY_KBN_DISP,
            uiEntryMst.getHonbuDispFrom(), uiEntryMst.getHonbuDispTo());

        // 本部：申込日登録
        insertEntryDate(uiEntryDate, UserType.HONBU,
            NationalRegistConstants.DAY_KBN_ENTRY, uiEntryMst.getHonbuEntryFrom(),
            uiEntryMst.getHonbuEntryTo());

        /* 全国大会セレクション情報登録 */
        UIEntrySelection uiEntrySelection = new UIEntrySelection();
        // エントリーコード、年、回設定
        uiEntrySelection.setEntryCd(EntryCode.NATIONAL);
        ...
        insertEntrySelection(uiEntrySelection, dto.getEntryKbnList(),
            NationalRegistConstants.SEL_KBN_ENTRY);

        // オプショナル登録
        insertEntrySelection(uiEntrySelection, dto.getOptionalList(),
            NationalRegistConstants.SEL_KBN_OPTIONAL);
    }
     ...
     * @param dto 全国大会マスタ登録情報DTO
     */
    private void executeUpdate(NationalRegistDto dto) {
        // エントリー年 = 開催開始日の年度
        String entryYear = createEntryYear(dto.getUiEntryMst().getOpenFrom());
        // エントリー回取得
        String entryKai = createEntryKai(EntryCode.NATIONAL, entryYear);
        ...
        if (entryYear.equals(uiEntryMst.getEntryYear())) {
            ...
            int result = getUiEntryMstDao().updatePKey(
                uiEntryMst, entryYear, entryKai, DateManager.getCurrentTimestamp());
            // 排他チェック
            if (result == 0) {
                throw new CannotExecuteWithReasonException(
                    NationalRegistConstants.MSG_EXCLUSION_CHK,
                    NationalRegistConstants.MSG_UPDATE);
            }
        }
     ...
     * @param dto 全国大会マスタ登録情報DTO
     */
    private void executeDelete(NationalRegistDto dto) {
        // 全国大会マスタ情報
        UIEntryMst uiEntryMst = dto.getUiEntryMst();
        // 修正ユーザ設定
        uiEntryMst.setLastUser(dto.getUserId());
        ...
        // 削除フラグ設定
        uiEntryMst.setSakujoFlg(NationalRegistConstants.SAKUJO_FLG_ON);
        // update
    ...
    private String createEntryYear(String openFromDt) {
        try {
            ...
            throw new FtlSystemException(NationalRegistConstants.MSG_ERR_ENTRY_YEAR);
        }
    }
    ...
    private String createEntryKai(String entryCd, String entryYear) {
        ...
        formatter.setFormatPattern(NationalRegistConstants.CF_ENTRY_KAI);
    ...
    private void updateEntryDate(
        ...
        for (int i = 0; i < dateInfoList.size(); i++) {
            ...
            // 開催開始終了日設定
            if (UserType.KANRI.equals(dateInfo.getUsertypeCd())
                && NationalRegistConstants.DAY_KBN_OPEN.equals(dateInfo.getDayKbn())) {
                dateInfo.setFromDt(uiEntryMst.getOpenFrom());
                dateInfo.setToDt(uiEntryMst.getOpenTo());
            // オーナー：表示開始終了日設定
            } else if (UserType.ONER.equals(dateInfo.getUsertypeCd())
                && NationalRegistConstants.DAY_KBN_DISP.equals(dateInfo.getDayKbn())) {
                dateInfo.setFromDt(uiEntryMst.getOnerDispFrom());
                dateInfo.setToDt(uiEntryMst.getOnerDispTo());
            // オーナー：申込開始終了日設定
            } else if (UserType.ONER.equals(dateInfo.getUsertypeCd())
                && NationalRegistConstants.DAY_KBN_ENTRY.equals(dateInfo.getDayKbn())) {
                dateInfo.setFromDt(uiEntryMst.getOnerEntryFrom());
                dateInfo.setToDt(uiEntryMst.getOnerEntryTo());
            // 本部：表示開始終了日設定            
            } else if (UserType.HONBU.equals(dateInfo.getUsertypeCd())
                && NationalRegistConstants.DAY_KBN_DISP.equals(dateInfo.getDayKbn())) {
            ...
            if (entryYear.equals(uiEntryMst.getEntryYear())) {
                ...
                int result = getUiEntryDateDao().updatePKey(
                    dateInfo, entryYear, entryKai, DateManager.getCurrentTimestamp());
                // 排他チェック
                if (result == 0) {
                    throw new CannotExecuteWithReasonException(
                            NationalRegistConstants.MSG_EXCLUSION_CHK,
    ...
    private void insertEntrySelection(
        ...
        formatter.setFormatPattern(NationalRegistConstants.CF_SEL_INDEX);
        ...
        for(int i = 0; i < selectionList.size(); i++) {
            ...
            if(CommonUtil.isNull(info.getSelectionName())) {
            	info.setSelectionName("");
            }
    ...
    private void updateEntrySelection(List selectionList, String userId,
        ...
        for (int i = 0; i < selectionList.size(); i++) {
            ...
            if (entryYear.equals(info.getEntryYear())) {
                ...
                int result = getUiEntrySelectionDao().updatePKey(
                    info, entryYear, entryKai, DateManager.getCurrentTimestamp());
                // 排他チェック
                if (result == 0) {
                    throw new CannotExecuteWithReasonException(
                            NationalRegistConstants.MSG_EXCLUSION_CHK,
