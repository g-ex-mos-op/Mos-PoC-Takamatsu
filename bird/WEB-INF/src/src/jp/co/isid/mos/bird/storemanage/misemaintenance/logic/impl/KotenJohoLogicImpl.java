...
import jp.co.isid.mos.bird.storemanage.misemaintenance.dao.CodEventPatternCdDao;
import jp.co.isid.mos.bird.storemanage.misemaintenance.dao.CodInsentiveCdDao;
...
import jp.co.isid.mos.bird.storemanage.misemaintenance.dao.MstGyotaiDao;
import jp.co.isid.mos.bird.storemanage.misemaintenance.dao.MstHDCDao;
import jp.co.isid.mos.bird.storemanage.misemaintenance.dao.MstInsentiveDao;
...
public class KotenJohoLogicImpl implements KotenJohoLogic {
    ...
    /* ロジックID */
    public static final String LOGIC_ID = "BSM001L02";
    ...
    //20060518 追加
    private CodEventPatternCdDao codEventPatternCdDao;
    ...
    public Map execute(String miseCd, String companyCd) throws ApplicationException {
        ...
        //20060518 追加 start ---------------------------------------------------------------
        /* １５．DAO【イベントパターンコード．イベントパターンコードの検索】を実行 */

        setListEventPatternCd(listMstEventStatus, listMstGyotai);
//        map.put("listEventPatternCd", listEventPatternCd);
        //20060518 追加 end -----------------------------------------------------------------
    ...
    /**
     * 2006/05/18 追加
     * イベントパターンコードリストを設定します。
     * @param listMstEventStatus [[イベント実施状況]]
     * @param listMstGyotai      [[業態履歴]]
     */
    private void setListEventPatternCd(List listMstEventStatus, List listMstGyotai){
        List listEventPatternCd = new ArrayList();
           
        String gyotai_cd = "";
        if (listMstGyotai != null && listMstGyotai.size() != 0) {
            gyotai_cd = ((MstGyotai) listMstGyotai.get(0)).getGyotaiKbn();
        }
             
        for(int i = 0; i < listMstEventStatus.size(); i++){
            MstEventStatus entity = (MstEventStatus)listMstEventStatus.get(i);
            // イベントパターンコード取得
            listEventPatternCd = getCodEventPatternCdDao().selectEventPattern(entity.getEventCd(), gyotai_cd);
            entity.setListEventPatternCd(listEventPatternCd);
        }
    }
...
//20050618 追加 start ----------------------------------------------
    /**
     * @return codEventPatternCdDao を戻します。
     */
    public CodEventPatternCdDao getCodEventPatternCdDao() {
        return codEventPatternCdDao;
    }
    /**
     * @param codEventPatternCdDao codEventPatternCdDao を設定。
     */
    public void setCodEventPatternCdDao(CodEventPatternCdDao codEventPatternCdDao) {
        this.codEventPatternCdDao = codEventPatternCdDao;
    }
//20050618 追加 end -----------------------------------------------
