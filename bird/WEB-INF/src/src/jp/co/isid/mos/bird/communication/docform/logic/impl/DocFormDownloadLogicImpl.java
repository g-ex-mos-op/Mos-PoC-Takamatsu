...
/**
 * 文書・フォーム ファイルダウンロードロジック
 * @author xnkusama
 *
 */
public class DocFormDownloadLogicImpl implements DownloadLogic {

    /* ロジックID */
    public static final String LOGIC_ID = "BCM004L01";
    ...
    public String getFileName(final DownloadDto downloadDto) {
        DocFormDto dto = (DocFormDto) downloadDto;
        String fileName = "";
        switch (getDownloadMode(downloadDto)) {
            case 1:
                //UIViewBunshoInfo entity = dto.getEntityViewDetail();
                UIViewDocFormInfo entity = (UIViewDocFormInfo) dto.getListDocForm().get(dto.getDownloadFileIndex());
                // 文書ファイル
                fileName = entity.getFileName();
                break;
            case 3:
                int viewIndex = dto.getViewSelectedAttachFileIndex();
                UIViewDocFormInfo viewTargetEntity = (UIViewDocFormInfo)dto.getListDocForm().get(dto.getIndexEntityViewDetail());
    ...
    public String getFileFullPath(final DownloadDto downloadDto) {
        ...
        switch (getDownloadMode(downloadDto)) {
            case 1:
                 //UIViewBunshoInfo entity = dto.getEntityViewDetail();
                 UIViewDocFormInfo entity = (UIViewDocFormInfo) dto.getListDocForm().get(dto.getDownloadFileIndex());
                 path = getFileRootPath(entity.getInfoShu());
                // 文書ファイル
            ...
            case 3:
                ...
                path = getFileRootPath(viewTargetEntity.getInfoShu());
                ...
                if(viewIndex == 2){
                    ...
                    //関連文書のパス取得
                    path = getFileRootPath(entityKanren.getInfoShu());
                    break;
                }else{
                    break;
                }
     ...
     * @return パス
     */
    private String getFileRootPath(String infoShu){
        //通達
        if(INFO_SHU_TUTATU.equals(infoShu)){
            return BirdContext.getProperty("fileProperty", "filePathTutatu");

        //文書
        }else if(INFO_SHU_DOC.equals(infoShu)){
