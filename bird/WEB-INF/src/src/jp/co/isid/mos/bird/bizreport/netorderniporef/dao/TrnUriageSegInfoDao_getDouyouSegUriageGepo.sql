     ...
     from
        ...
        join
            (select
                 ...
                 max(bd53.open_kbn_zen_doyo) as open_kbn_zen,
                 max(bd53.misecnt_doyo_nsum) as misecnt_kbn_nsum,
                 max(bd53.misecnt_doyo_ntake) as misecnt_kbn_ntake,
                 max(bd53.misecnt_doyo_ntaku) as misecnt_kbn_ntaku,
                 sum(bd53.uriage_doyo) as uriage,
                 sum(bd53.kyakusu_doyo) as kyakusu,
                 sum(bd53.uriage_zen_doyo) as uriage_zen,
                 sum(bd53.kyakusu_zen_doyo) as kyakusu_zen,
                 sum(bd53.uriage_doyo_nsum) as uriage_nsum,
                 sum(bd53.kyakusu_doyo_nsum) as kyakusu_nsum,
                 sum(bd53.uriage_zen_doyo_nsum) as uriage_nsum_zen,
                 sum(bd53.kyakusu_zen_doyo_nsum) as kyakusu_nsum_zen,
                 sum(bd53.uriage_doyo_ntake) as uriage_ntake,
                 sum(bd53.kyakusu_doyo_ntake) as kyakusu_ntake,
                 sum(bd53.uriage_zen_doyo_ntake) as uriage_ntake_zen,
                 sum(bd53.kyakusu_zen_doyo_ntake) as kyakusu_ntake_zen,
                 sum(bd53.uriage_doyo_ntaku) as uriage_ntaku,
                 sum(bd53.kyakusu_doyo_ntaku) as kyakusu_ntaku,
                 sum(bd53.uriage_zen_doyo_ntaku) as uriage_ntaku_zen,
                 sum(bd53.kyakusu_zen_doyo_ntaku) as kyakusu_ntaku_zen
             from
                 BD53NGEP as bd53
                 join
             ...
             /*IF tenShu.equals("ALL")*/
                 and (bd53.open_kbn = 1 or bd53.open_kbn_zen_doyo = 1)
             -- ELSE
                 and bd53.open_kbn = 1
             /*END*/
             and bd53.oldm_flg = '0'
