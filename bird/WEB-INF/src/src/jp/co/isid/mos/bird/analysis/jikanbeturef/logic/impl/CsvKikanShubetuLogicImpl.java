...
*
*/
public class CsvKikanShubetuLogicImpl implements CsvOutputLogic {
/** ロジックID */
public static final String LOGIC_ID = JikanBetuRefUtil.LOGIC_ID_CSV_SHUBETU_KIKAN;
...
//『月別』の場合
return "JIKANURISHU"+reqestDto.getKikanSitei()+"_"+ reqestDto.getKikanSiteiTo()+ ".csv";
}
...
private void addHeader(List listCSV, MenuBetuReqDto requestDto) {
   ...
   if(TaishoKikan.DAY.equals(requestDto.getTaishoKikan())) {
    listHeader2Row.add(MenubetuUtil.formattYMDSlash(requestDto.getKikanSitei()));
       ...
       listHeader2Row.add(MenubetuUtil.formattYMDSlash(requestDto.getKikanSitei()) + "～"+MenubetuUtil.formattYMDSlash(requestDto.getKikanSiteiTo()) );
   }else {
...
private void addList(List listCSV, List listCsvData) {
   for(int i=0; i<listCsvData.size(); i++) {
       UIWeekShubetuKikan entity = (UIWeekShubetuKikan)listCsvData.get(i);
       ...
       if(RowType.CD_KOUSEIHI.equals(entity.getRowType())) {
           ...
           rowData.add(numFmtdgt2.format(entity.getKingakuAll()));
           rowData.add("***");//時間帯構成比
...
//           rowData.add(numFmtdgt2.format(entity.getKyakusuEtc()));
       ...
       rowData.add(numFmtdgt0.format(entity.getKingakuAll()));
       rowData.add(Calculator.percentage(entity.getKingakuAll(), total, 2));
