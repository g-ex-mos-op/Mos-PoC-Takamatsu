 ...
 * @author Aspac
 */
public class CheckPLDataNoInputLogicImpl implements CheckPLDataNoInputLogic {

    /* ロジックID */
    private static final String LOGIC_ID = "BBS004L04";
    ...
    public void execute(HashMap hashMap) {
        ...
        TrnPLInfoList trnPLInfo = null;
        trnPLInfo = (TrnPLInfoList) hashMap.get("PL_DATA");
        
        // 各種DBデータを保持　レスポンス対策★
        List plLimitList = (List) hashMap.get("PL_LMIT");
        List listSales = (List) hashMap.get("PL_SALE");
        List listPrev  = (List) hashMap.get("PL_PREV");
        ...
        if (trnPLInfo.getPlType().equals("1")) {
            ...
            // ★パラメータの売上げリストからチェック対象のデータを検索
            BigDecimal sales = new BigDecimal(0);
            for (Iterator it = listSales.iterator(); it.hasNext();) {
                TrnUriageInfo trnUriage = (TrnUriageInfo) it.next();
                if(trnUriage.getMiseCd().equals(trnPLInfo.getMiseCd()) && trnUriage.getCompanyCd().equals(trnPLInfo.getCompanyCd()))
                    sales = trnUriage.getUriage(); 
            }
            if (sales == null) {
        ...
        // 前月在庫情報取得
        TrnPLInfo prevTrnPLInfo = getPrevPlInfo(trnPLInfo, listPrev);
     ...
     */
    protected TrnPLInfo getPrevPlInfo(TrnPLInfo trnPLInfo, List listPrev) {

        TrnPLInfo prevTrnPLInfo = null;
        String miseCd = trnPLInfo.getMiseCd();
        String compCd = trnPLInfo.getCompanyCd();
        boolean isPrevFlg = false;
        
        // ★前月P/Lデータリストからチェック対象データを検索
        for (Iterator it = listPrev.iterator(); it.hasNext();) {
            prevTrnPLInfo = (TrnPLInfo) it.next();
            
            if (prevTrnPLInfo.getMiseCd().equals(miseCd) && prevTrnPLInfo.getCompanyCd().equals(compCd)) {
                isPrevFlg = true;
                break;
            }
        }
