...
public class SearchEntryLogicImpl implements SearchEntryLogic {
    /** 全国大会マスタ情報取得ロジックID */
    public static final String LOGIC_ID = "BEN014L02";
    ...
    public Map execute(
        ...
        if (dateList == null || dateList.size() != dateSize) {
            throw new FtlSystemException(NationalRegistConstants.MSG_ERR_DATE_INFO);
        }

        // 日付情報リスト設定
        result.put(NationalRegistConstants.MAP_DATE_LIST, dateList);
        ...
        if (selectionList == null || selectionList.size() != selectionSize) {
            throw new FtlSystemException(NationalRegistConstants.MSG_ERR_SELECTION_INFO);
        }
 
        // セレクション情報リスト設定
        result.put(NationalRegistConstants.MAP_SELECTION_LIST, selectionList);
    ...
    private void validate(String entryCd, String entryYear, String entryKai) {
        // エントリーコード
        if (NationalRegistCommon.isNull(entryCd)) {
            throw new NotNullException(NationalRegistConstants.MSG_ENTRY_CD);
        }
        // エントリー年
        if (NationalRegistCommon.isNull(entryYear)) {
            throw new NotNullException(NationalRegistConstants.MSG_ENTRY_YEAR);
        }
        // エントリー回
        if (NationalRegistCommon.isNull(entryKai)) {
            throw new NotNullException(NationalRegistConstants.MSG_ENTRY_KAI);
        }
    }
