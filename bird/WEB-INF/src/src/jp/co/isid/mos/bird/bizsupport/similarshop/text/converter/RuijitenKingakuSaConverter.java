 ...
 *
 */
package jp.co.isid.mos.bird.bizsupport.similarshop.text.converter;
 ...
 * @author xytamura
 */
public class RuijitenKingakuSaConverter extends NormalConverter implements Converter, Formatter {

	private RuijitenReferenceDto ruijitenReferenceDto;
    /**
     * コンストラクタ デフォルトのパターン
     */
    public RuijitenKingakuSaConverter() {
        super();
    }
    ...
    public String getAsString(FacesContext facesContext,
        ...
        BigDecimal kinZen = new BigDecimal("0");
        String strKinZen = jp.co.isid.mos.bird.framework.util.Converter.decToString(new BigDecimal(zennen), 0);
        if(strKinZen != null && !strKinZen.equals("")){
        	try{
        		kinZen = jp.co.isid.mos.bird.framework.util.Converter.stringToDec(strKinZen);
        	}catch (Exception ex){
        		//何もしない
        	}
        }
        kosehi = kinTou.subtract(kinZen);
        
        return formatter.format(kosehi.toString());
    }
