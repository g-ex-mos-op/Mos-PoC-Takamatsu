...
left outer join (
		SELECT
		  SUBBT59.EIGYO_DT,
		  SUM(COALESCE(SUBBT59.URIAGE_ZEN_AFTER_NEBIKI,0)) AS URIAGE_ZEN_AFTER_NEBIKI
		FROM
...
/*END*/
            ...
            ,    BN01DTEN BN01
            ,   (SELECT CASE WHEN SUBSTR(BT59.EIGYO_DT, 7,2)='99'
                                THEN SUBSTR(BT59.EIGYO_DT, 1,6) || '28'
                               WHEN SUBSTR(BT59.EIGYO_DT, 7,2)='28'
                                THEN SUBSTR(BT59.EIGYO_DT, 1,6) || '00'
                           ELSE BT59.EIGYO_DT END AS BN01_EIGYO_DT
                   ,      BT59.EIGYO_DT
	               ,      BT59.COMPANY_CD
	               ,      BT59.MISE_CD
		           ,       COALESCE(BT59.URI_ZEN_AFT_NBK_DOGETU,0) AS URIAGE_ZEN_AFTER_NEBIKI
	               from BT59ANZJ BT59
	               WHERE BT59.COMPANY_CD = /*companyCd*/'00'
	                   AND   BT59.EIGYO_DT < /*sysDate*/'20090302'
	                   AND   SUBSTR(EIGYO_DT, 5,4) IN ('0228','0299')
	                   AND   SUBSTR(BT59.EIGYO_DT, 1, 6) = /*taishoYm*/'20110101'
	              )SUBBT59
		WHERE
             SUBSTR(SUBBT59.BN01_EIGYO_DT, 5,4) = '0228'
                         AND   SUBBT59.COMPANY_CD    = BN01.COMPANY_CD
            AND   SUBBT59.BN01_EIGYO_DT = BN01.EIGYO_DT
            AND   SUBBT59.MISE_CD       = BN01.MISE_CD
/*IF tenpoShubetu == "1" || tenpoShubetu == "3"*/
            AND   BN01.KBN1 = /*tenpoShubetu*/''
/*END*/
/*IF tenpoShubetu == "2" */
            AND   BN01.KBN1 IN ('1', '2')
/*END*/
            AND   SUBBT59.MISE_CD  = BM01.MISE_CD
		GROUP BY
		  SUBBT59.EIGYO_DT
) AS BT59 ON (BT59.EIGYO_DT = BT60.EIGYO_DT)
/*END*/
