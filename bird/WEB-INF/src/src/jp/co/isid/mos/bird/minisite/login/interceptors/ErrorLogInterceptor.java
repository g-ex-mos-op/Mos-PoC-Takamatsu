...
public class ErrorLogInterceptor extends ThrowsInterceptor {
    ...
    public String handleThrowable(
    	...
    	if(t instanceof MinisiteIllegalAccessException) {
            ...
            /**
             * リクエスト情報ログ出力(仕様番号:SP010301)
             *
             * パラメータ必須チェック、パラメータ妥当性チェック（画面）、パラメータ妥当性チェック（時刻）、
             * ユーザーIDの存在チェックでNGとなった場合は、下記内容をファイルに出力する
             * ユーザーエージェント
             * ＩＰアドレス
             * リクエストパラメータ値
             */
            ...
            //エラー内容
            buf.append(" " + ((MinisiteIllegalAccessException)t).getMemo() +"エラー");
            //ユーザーエージェント
            ...
            if (request.getSession() != null ) {
    	        String minisiteLoginRequestURL =
    	        	(String) request.getSession().getAttribute(LoginAction.PK_FIRSTURL);
    	        if(minisiteLoginRequestURL != null) {
    		        //パラメータ値の設定
    	        	buf.append(" RequestURL: [" + minisiteLoginRequestURL + "]");
