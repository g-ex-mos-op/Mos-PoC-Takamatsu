...
import jp.co.isid.mos.bird.bizreport.moscardniporef.action.MoscardNipoRefSibuConfirmAction;
...
/**
 * MOSCARDの結果画面(支部別)アクション
 *
 * @author   xyamauchi
 */
public class MoscardNipoRefSibuConfirmActionImpl extends CsvOutput2ActionImpl implements MoscardNipoRefSibuConfirmAction {
    /** 初期アクションID */
    public static final String initialize_ACTION_ID  = "BBR015A05";

    /** 再検索アクションID */
    public static final String reSearch_ACTION_ID = "BBR015A06";

    /** アクションID:アクション【CSVダウンロード(支部別)】 */
    public static final String downloadCsvSibu_ACTION_ID = "BBR015A07";

    /** アクションID:アクション【CSVダウンロード(店舗別)】 */
    public static final String downloadCsvMise_ACTION_ID = "BBR015A08";

    /** 支部リンクアクションID */
    public static final String selectedSibu_ACTION_ID = "BBR015A09";

    /** 宅配売上日報タブリンクアクションID */
    public static final String goTakuhaiNipo_ACTION_ID = "BBR015A11";

    /** 屋号別売上タブリンクアクションID */
    public static final String goYago_ACTION_ID = "BBR015A12";

    /** アクション【営業日報画面遷移】ID */
    public static final String goEigyoNipo_ACTION_ID = "BBR015A10";

    /** アクション【SV検索ボタン】ID */
    public static final String callSvForm_ACTION_ID   = "BBR015A13";

    /** アクション【ネット注文画面遷移】ID */
    public static final String callNetorder_ACTION_ID = "BBR015A14";
    ...
    /** LOGIC【CSVダウンロード(支部別)】 */
    private CsvOutputLogic moscardNipoRefCsvOutputSibuLogic;

    /** LOGIC【CSVダウンロード(店舗別)】 */
    private CsvOutputLogic moscardNipoRefCsvOutputMiseLogic;
    ...
    /** 表示売上LOGIC【予算情報取得】 */
    private ViewUriYosanLogic moscardNipoRefViewUriYosanLogic;
    ...
    public String initialize() {
        ...
        //２．SV選択画面から遷移してきた場合。2008/12/09追加 **************************/
            ...
            if(getNewSvSearchDto().getReturnKind() == SvSearchDto.RETURNKIND_SELECT){
                //SV-SELECT-1.店舗別画面VIEW_IDをリターンします。(MOSCARD店舗別画面へ遷移)
    ...
    public String reSearch() {
        ...
        if(searchParamDto.isSvFlg()){
            ...
            //SV-SELECT-3.MOSCARD店舗別画面VIEW_IDをリターンします。(店舗別画面へ遷移)
            return NipoRefConstants.VIEW_ID_M_MISE;
        }
        //３．支部別MOSCARD売上情報を実行します。
    ...
    public String callSvForm()  {
        //１．BIRD共通DTO【SV選択画面】.遷移元情報へMOSCARD支部別画面VIEW_IDを設定します。
        getNewSvSearchDto().setNavigationCase(NipoRefConstants.VIEW_ID_M_SIBU);
    ...
    public void downloadCsvSibu() {
        try {
        	...
        	setCsvOutputLogic(getMoscardNipoRefCsvOutputSibuLogic());
    ...
    public void downloadCsvMise() {
        try {
        	...
        	setCsvOutputLogic(getMoscardNipoRefCsvOutputMiseLogic());
        	//４．継承クラス.ダウンロード メイン処理を実行します。
            super.downloadCsv();
        } catch (IOException e) {
        	//５．IOExceptionが発生した場合、FtlSystemExceptionを発生させます。
    ...
    public String goTakuhaiNipo() {
        ...
        // ２．遷移先のサブタブの位置を0に設定
        resultParamDto.setSubTagKbn(BizReportConstants.SUB_TAG_0);
        //３．宅配売上日報・支部別結果画面VIEW_IDをリターンします。
    ...
    public String goYago() {
        ...
        //２．遷移先のサブタブの位置を0に設定
        resultParamDto.setSubTagKbn(BizReportConstants.SUB_TAG_0);
        //３．屋号別売上・支部別結果画面VIEW_IDをリターンします。
    ...
    public String callNetorder() {
        ...
        NipoRefUtil.callScreenInitialize(
        		getNipoRefConditionDto(), searchParamDto, resultParamDto, isNewWindowFlg());
        //３．ネット注文・支部別結果画面VIEW_IDをリターンします。
     ...
     * 支部別MOSCARD売上情報を取得する
     *
     * リモート制限ユーザログイン時閲覧可能担当支部存在チェックを行う処理有り。
     * 更新日:2013/06/03 ADD xkinu
     *        前年データ種別『前年同月営業日補正』時に本部発行の実績値が取得されていなかった。
     *        売上情報と同じに『前年同月』のデータも取得するよう修正。
     *
    ...
    private void searchSibuUriageInfo(NipoRefConditionParameterDto parameterDto){
        ...
        try {
            ...
            //7.変数List[[本部売上情報]]を生成します。
            List honbuUriageList = new ArrayList();
	        //8.Map[検索情報].前年データ種別＝”前年同月営業日補正”の場合、下記の処理を行います。
	        if (NipoZennenDataShubetu.CODE_HOSEI.equals((String) paramMap.get(MoscardNipoConstants.MAP_DATASHU))) {
	            ...
	            //変数List[[売上情報]]が0件の場合NoResultExceptionを発生させます。
	            if(uriageList.size() == 0) {
	                throw new NoResultException(MoscardNipoConstants.EMPTY);
	            }
	            //補正処理2-1.LOGIC【本部売上情報取得】を実行し、戻り値を処理7の変数List[[本部売上情報]]へ設定します。
	            ...
	            //補正処理6.LOGIC【本部売上情報取得】を実行し、List[[補正データ]]を取得します。
	            List honbuDogetuUriageList =getHonbuUriageInfoLogic().execute(paramMap);
	            //補正処理7.補正処理6のList[[補正データ]]を変数Map[売上予算情報]へ"honbuHoseiUriage"をキーに格納します。
	            uriYosanMap.put(MoscardNipoConstants.MAP_HONBU_HOSEI_URIAGE_LIST, honbuDogetuUriageList);
	        }
	        //9.Map[検索情報].前年データ種別 <> ”前年同月営業日補正”の場合、下記の処理を行います。
	        else{
	        	//1.LOGIC【売上情報取得】を実行し、戻り値を変数List[[売上情報]]へ設定します。
	        	uriageList = getUriageInfoLogic().execute(paramMap);
	            //変数List[[売上情報]]が0件の場合NoResultExceptionを発生させます。
	            if(uriageList.size() == 0) {
	                throw new NoResultException(MoscardNipoConstants.EMPTY);
	            }
	            //2.LOGIC【本部売上情報取得】を実行し、戻り値を処理7の変数List[[本部売上情報]]へ設定します。
	            honbuUriageList = getHonbuUriageInfoLogic().execute(paramMap);
	        }


	        //10.変数Map[売上予算情報]へ支部情報リストキーで、変数List[[支部情報]]を格納します。
	        uriYosanMap.put(MoscardNipoConstants.MAP_SIBU_LIST, sibuList);
	        //11.変数Map[売上予算情報]へ売上情報リストキーで、変数List[[売上情報]]を格納します。
	        uriYosanMap.put(MoscardNipoConstants.MAP_URIAGE_LIST, uriageList);
	        //12.変数Map[売上予算情報]へ集計区分キーで、DTO【検索条件】.集計区分を格納します。
	        uriYosanMap.put(MoscardNipoConstants.MAP_SHUKEI_KBN, parameterDto.getShukeiKbnCd());
	        //13.変数Map[売上予算情報]へパラメータキーで、変数Map[検索条件]を格納します。
	        uriYosanMap.put(MoscardNipoConstants.MAP_PARAMS, paramMap);
            //14.変数Map[売上予算情報]へパラメータキーで、変数List[[本部売上情報]]を格納します。
            uriYosanMap.put(MoscardNipoConstants.MAP_HONBUURIAGE_LIST, honbuUriageList);
	        //15.LOGIC【表示売上予算情報取得】を実行し、戻り値を変数Map[表示売上予算情報]とします。
	        Map resultMap = getViewUriYosanLogic().execute(uriYosanMap);
	        //16.変数Map[表示売上予算情報]から売上リストキーでList[[画面表示用売上]]を取得します。
	        List listUriage = (List)resultMap.get(MoscardNipoConstants.MAP_URIAGE_LIST);
	        //17.List[[画面表示用売上]]が0件の場合、NoResultExceptionを発生させます。
	        if(listUriage.size() == 0) {
	            throw new NoResultException(MoscardNipoConstants.EMPTY);
	        }
	        //18.DTO【結果情報】.List[[売上情報]]へ処理16のList[[画面表示用売上]]を設定します。
	        moscardNipoRefResultDto.setUriageList(listUriage);
	        //19.DTO【結果情報】.List[[売上平均情報]]へ変数Map[表示売上予算情報]から売上平均リストキーの値Listを設定します。
	        moscardNipoRefResultDto.setUriAvgList((List)resultMap.get(MoscardNipoConstants.MAP_RST_URI_AVE));
	        //20.DTO【結果情報】.List[[客数情報]]へ変数Map[表示売上予算情報]から客数リストキーの値Listを設定します。
	        moscardNipoRefResultDto.setKyakusuList((List)resultMap.get(MoscardNipoConstants.MAP_KYAKUSU_LIST));
	        //21.DTO【結果情報】.List[[客数平均情報]]へ変数Map[表示売上予算情報]から客数平均リストキーの値Listを設定します。
	        moscardNipoRefResultDto.setKyakuAvgList((List)resultMap.get(MoscardNipoConstants.MAP_RST_KYAKUSU_AVE));
	        //22.DTO【結果情報】.対象店舗数へ変数Map[表示売上予算情報]から対象店舗数キーの値intを設定します。
	        moscardNipoRefResultDto.setTenpoCount(
	            ((BigDecimal) resultMap.get(MoscardNipoConstants.MAP_TENPO_COUNT)).intValue());
	        //23.DTO【結果情報】.該当データ存在判断フラグへtrueを設定する。
	        moscardNipoRefResultDto.setExistDataFlg(true);
	        //24.DTO【結果情報】.検索タイプへTaishoJoken.CODE_MISEを設定する。
	        moscardNipoRefResultDto.setSearchListType(TaishoJoken.CODE_SIBU);
	        //25.DTO【結果情報】換算フラグへDTO【検索条件】.換算フラグを設定する。
	        moscardNipoRefResultDto.setKansanFlg(parameterDto.isKansan());
	        //26.日報共通DTO【結果条件】へDTO【検索条件】を設定します。
	        setNipoRefResultParameterDto(parameterDto);
            //27.日報共通DTO【条件部情報】.日報共通DTO【結果条件】へDTO【検索条件】を設定します。
            getMoscardNipoRefMapResultDto().setParameterDto(parameterDto);
	        //28.DTO【Map結果情報】.DTO【結果情報】へDTO【検索条件】.ウィンドウIDをキーに、
	        //DTO【結果情報】を格納します。
	        getMoscardNipoRefMapResultDto().setResultDto(
	        		parameterDto.getWindowId(), moscardNipoRefResultDto);
	        //29.日報共通DTO【検索条件】.再検索制御フラグへtrueを設定します。
    ...
    /**
     * LOGIC【検索条件取得】を取得する
     * @return MoscardNipoSearchLogic LOGIC【検索条件取得】
     */
    ...
    /**
     *  LOGIC【検索条件取得】を設定する
     * @param searchLogic LOGIC【検索条件取得】
     */
    ...
    public ViewUriYosanLogic getViewUriYosanLogic() {
        return this.moscardNipoRefViewUriYosanLogic;
    }
     ...
     */
    public void setViewUriYosanLogic( ViewUriYosanLogic moscardNipoRefViewUriYosanLogic) {

        this.moscardNipoRefViewUriYosanLogic = moscardNipoRefViewUriYosanLogic;
    }

    /**
     * LOGIC【本部売上情報取得】を取得する
     * @return UriageInfoLogic LOGIC【売上情報取得】
     */
    ...
    /**
     *  LOGIC【売上情報取得】を設定する
     * @param honbuUriageInfoLogic LOGIC【売上情報取得】
     */
	...
	/**
	 * @return クラス変数numericConverter を戻します。
	 */
	...
	/**
	 * @param numericConverter を クラス変数numericConverterへ設定します。
	 */
	public void setNumericConverter(NumericConverter numericConverter) {
	...
	/**
	 * @return クラス変数moscardNipoRefCsvOutputMiseLogic を戻します。
	 */
	public CsvOutputLogic getMoscardNipoRefCsvOutputMiseLogic() {
		return moscardNipoRefCsvOutputMiseLogic;
	}

	/**
	 * @param moscardNipoRefCsvOutputMiseLogic を クラス変数moscardNipoRefCsvOutputMiseLogicへ設定します。
	 */
	public void setMoscardNipoRefCsvOutputMiseLogic(
			CsvOutputLogic moscardNipoRefCsvOutputMiseLogic) {
		this.moscardNipoRefCsvOutputMiseLogic = moscardNipoRefCsvOutputMiseLogic;
	}

	/**
	 * @return クラス変数moscardNipoRefCsvOutputSibuLogic を戻します。
	 */
	public CsvOutputLogic getMoscardNipoRefCsvOutputSibuLogic() {
		return moscardNipoRefCsvOutputSibuLogic;
	}

	/**
	 * @param moscardNipoRefCsvOutputSibuLogic を クラス変数moscardNipoRefCsvOutputSibuLogicへ設定します。
	 */
	public void setMoscardNipoRefCsvOutputSibuLogic(
			CsvOutputLogic moscardNipoRefCsvOutputSibuLogic) {
		this.moscardNipoRefCsvOutputSibuLogic = moscardNipoRefCsvOutputSibuLogic;
	}
