...
,               bm10.sibu_cd
,               bm10.sibu_name
,               (case when bt54.sibu_cd is null
                      then (case when bt45.sibu_cd is null then 'NOTDISP' 
                                 when bt45.yosan = 0      then 'NOTDISP'
                                 else 'DISPONLY' end)
                      else 'DISP'
                 end) as disp_kbn
from            (select * from bm10gsib where company_cd = /*companyCd*/'00') bm10
left join       (
...
on              (bm10.company_cd = bt54.company_cd and bm10.sibu_cd = bt54.sibu_cd)
left join       (
