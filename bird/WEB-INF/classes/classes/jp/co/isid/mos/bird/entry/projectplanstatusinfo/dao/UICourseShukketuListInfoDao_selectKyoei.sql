...
LEFT JOIN (
    SELECT BM11.SIBU_CD
    ,      BM11.ONER_CD
    ,      SUM(case when BT46.KYOEI_FLG = '0' then 1 else 0 end) as SHUSSEKI
    FROM BT46KENS BT46
    ,    (SELECT MISE.SIBU_CD
          ,      MISE.ONER_CD 
          FROM BM11ONER ONER
...
LEFT JOIN (
    SELECT BM11.ONER_CD
    ,      (case when MAX(case when BT46.KYOEI_FLG = '0' then 1 else 0 end) > 0 then 'Åõ' else 'Å~' end) as SHUSSEKI_KBN
    FROM BT46KENS BT46
    ,    BM11ONER BM11
    ,    BM01TENM BM01
