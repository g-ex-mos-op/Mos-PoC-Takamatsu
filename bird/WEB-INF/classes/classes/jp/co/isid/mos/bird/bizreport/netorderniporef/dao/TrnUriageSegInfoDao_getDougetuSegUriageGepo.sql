     ...
     from
        ...
        join  
            (select
                 ...
                 max(bd53.open_kbn_zen_dogetu) as open_kbn_zen,
                 max(bd53.misecnt_dogetu_nsum) as misecnt_kbn_nsum,
        		 max(bd53.misecnt_dogetu_ntake) as misecnt_kbn_ntake,
                 max(bd53.misecnt_dogetu_ntaku) as misecnt_kbn_ntaku,  
                 sum(bd53.uriage_dogetu) as uriage,
                 sum(bd53.kyakusu_dogetu) as kyakusu,
                 sum(bd53.uriage_zen_dogetu) as uriage_zen,
                 sum(bd53.kyakusu_zen_dogetu) as kyakusu_zen,
                 sum(bd53.uriage_dogetu_nsum) as uriage_nsum,
                 sum(bd53.kyakusu_dogetu_nsum) as kyakusu_nsum,
                 sum(bd53.uriage_zen_dogetu_nsum) as uriage_nsum_zen,
                 sum(bd53.kyakusu_zen_dogetu_nsum) as kyakusu_nsum_zen,
                 sum(bd53.uriage_dogetu_ntake) as uriage_ntake,
                 sum(bd53.kyakusu_dogetu_ntake) as kyakusu_ntake,
                 sum(bd53.uriage_zen_dogetu_ntake) as uriage_ntake_zen,
                 sum(bd53.kyakusu_zen_dogetu_ntake) as kyakusu_ntake_zen,
                 sum(bd53.uriage_dogetu_ntaku) as uriage_ntaku,
                 sum(bd53.kyakusu_dogetu_ntaku) as kyakusu_ntaku,
                 sum(bd53.uriage_zen_dogetu_ntaku) as uriage_ntaku_zen,
                 sum(bd53.kyakusu_zen_dogetu_ntaku) as kyakusu_ntaku_zen
             from
                BD53NGEP as bd53
                 join
