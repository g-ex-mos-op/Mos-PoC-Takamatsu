...
本文書は、ネット注文帳票機能に関するDao設計書です。ネット注文帳票情報を取得するためのデータアクセスオブジェクト（NetOrderReportDataDao）の設計を定義しています。主な内容として、エンティティ定義（NetOrderReport）、Dao情報、SQL検索メソッド（selectNetOrderReport）のパラメータ情報、使用テーブル、JOIN情報、条件、ソート情報が含まれています。
...
| JOIN情報 | 営業日報（日次）<br>BT63SNIP | 前年営業日情報<br>BM45ZDAY | グループ統合オーナマスタ<br>BM11ONER | 店統合マスタ<br>BM01TENM | 汎用店舗グループ（支部）<br>BM10GSIB | ネット注文帳票除外店舗マスタ<br>BM82NOJT |
|----------|------------------------------|----------------------------|--------------------------------------|--------------------------|--------------------------------------|----------------------------------------|
...
| カラム | - | - | - | エリア大コード | ⇔支部コード | - |
| カラム | - | - | オーナーコード | ⇔オーナーコード | - | - |
| カラム | - | 会社コード, 店舗コード | - | - | - | NOT IN (会社コード, 店舗コード) |
...
- **.xls形式のため画像の抽出ができませんでした**: xlrdライブラリでは画像抽出がサポートされていないため、シート内に画像が含まれている場合は抽出できていません。
- **結合セルのレイアウト**: 一部の結合セル（特にJOIN情報テーブル）は、元のExcelレイアウトを完全に再現できていない可能性があります。
- **日付データの変換**: Excelのシリアル値（43349.0など）から日付への変換を行いましたが、一部の日付が正確でない可能性があります。

## 不明点・不明瞭な点

- **SQL列の内容**: NetOrderReportDataDaoシートのパラメータ情報セクションに「SQL」列がありますが、具体的なSQL文の内容は空欄または別シートに記載されている可能性があります。
- **説明列の内容**: プロパティ定義テーブルの「説明」列は、元のExcelでは結合セルとして存在していますが、具体的な説明内容は空欄でした。
- **エンティティ出力先のパス**: `C:\Temp`というパスが設定されていますが、これが開発環境固有の設定か、本番環境でも使用されるかは不明です。
